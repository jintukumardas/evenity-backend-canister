 ...
 */
$query;
export function getNFTsForUser(userId: string): Result<Vec<NFT>, string> {
    // Validate IDs
    // if (!isValidUUID(eventId) || !isValidUUID(userId)) {
    //     return Result.Err<Vec<NFT>, string>('Invalid event ID or user ID');
    // }
    try {
        const nfts = nftStorage.values().filter(nft => nft.owner === userId);
        return Result.Ok(nfts);
    } catch (error) {
        return Result.Err('Failed to get NFTs for the user');
    }
}
...
/**
 * Transfers an NFT ticket from one user to another.
 * @param nftId - The ID of the NFT to transfer.
 * @param fromUserId - The ID of the current owner.
 * @param toUserId - The ID of the new owner.
 * @returns A Result containing the transferred NFT or an error message.
 */
$update;
export function transferNFT(nftId: string, fromUserId: string, toUserId: string): Result<NFT, string> {
    // Validate IDs
    if (!isValidUUID(nftId)) {
        return Result.Err<NFT, string>('Invalid NFT ID or user IDs for transferring NFT.');
    }

    return match(nftStorage.get(nftId), {
        Some: (nft) => {
            // Validate ownership
            if (nft.owner !== fromUserId) {
                return Result.Err<NFT, string>('Only the owner can transfer this NFT!');
            }
            ...
            // Update the NFT in nftStorage
            nftStorage.insert(nft.id, updatedNFT);

            return Result.Ok<NFT, string>(updatedNFT);
        },
        None: () => Result.Err<NFT, string>(`NFT with the provided id: ${nftId} has not been found!`),
    });
}
    ...
    return match(eventStorage.get(eventId), {
        Some: (event) => {
            ...
            const updatedEvent: Event = {
                ...
                speakers: payload.speakers || event.speakers,
